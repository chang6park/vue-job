<template>
    ...
    <div class="form-container" v-if="isLogin">
      <form @submit.prevent="handleSubmit">
        ...
        <!-- 3.시급, 월급 항목 선택 -->
        <div class="form-group">
          <input type="radio" id="pay_rule1" name="pay_rule" value="시급" v-model="pay_rule">
          <input type="radio" id="pay_rule2" name="pay_rule" value="월급" v-model="pay_rule">
          <div class="tab-group">
        ...
        <button class="btn-submint">등록하기</button>
      </form>
    </div>
  </template>
  ...
  <script setup>
    ...
    const handleSubmit = async () => {
      ...
      if (!pay_rule.value) {
        alert('시급 또는 월급을 선택해주세요.');
        return;
      }

      if (!pay.value) {
        alert('시급 또는 월급 금액을 입력해주세요.');
        return;
      }
