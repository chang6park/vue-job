<template>
  <ul class="job-list" v-if="posts.length > 0"> <!--posts 데이터가 들어왔을때 화면을 보여주는 조건문-->
    <li class="job-item" v-for="post in posts" :key="post.id"> <!-- // 배열 반복문으로 표시 / post.id가 글 번호 -->
...
<script setup>
  ...
  onMounted(async () => {
    const { data, error } = await supabase
      .from('job_posts')    // job_posts테이블에서 가져옴
      .select('*')
